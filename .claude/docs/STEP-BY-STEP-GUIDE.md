...
[0] Collect inputs
    ↓
[1] Business research (if name provided)
    ↓
[2] Location discovery → locations.json
    ↓
[3] Service schema → service-schema-template.json
    ↓
[4] State compliance (if YMYL) → state-compliance/*.json
    ↓
[5] Design generation (NOW HAS ALL DATA!)
    → design-system-generator → design-homepage → design-city-page → design-enhancer
    ↓
[6] Generate city pages (N agents in PARALLEL) → city-pages/*.json
    ↓
[7] Build NextJS site (nextjs-orchestrator → 7 sub-agents)
    → nextjs-core-builder → homepage-builder → service-pillar-builder
    → state-page-builder → city-page-builder → legal-pages-builder
    → lead-capture-builder
    ↓
[8] GitHub deployment
    ↓
[9] Report results
